...
public class ThirdPersonController_Student : MonoBehaviour
{
	...
	private const float groundedDistance = 0.75f;
	...
	// Handle movement here since physics will only be calculated in fixed frames anyway
	{
		...
		//Debug.Log ((target.transform.position + target.transform.up * -groundedCheckOffset).ToString () + " " + groundedDistance.ToString());
